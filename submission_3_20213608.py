...
def submission_3_20213608(yaml_path, output_json_path):
    data_config = load_yaml_config(yaml_path)
    model_name = 'lightweightob'
    ex_dict = {}
    epochs = 20
    batch_size = 8
    optimizer = 'SGD'
    ...
    model = YOLO('models/LightWeightOB/lightweightob.yaml', verbose=False)
    os.makedirs(output_dir, exist_ok=True)
    ex_dict['Model Name'] = model_name
    ex_dict['Model'] = model

    ex_dict = LightWeightOB.train_model(ex_dict)
